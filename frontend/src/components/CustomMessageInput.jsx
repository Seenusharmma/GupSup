...
const CustomMessageInput = ({ quotedMessage, clearQuotedMessage }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      const isInsideInput =
        inputZoneRef.current && inputZoneRef.current.contains(event.target);
    ...
    <div ref={inputZoneRef} className="flex flex-col w-full relative">
      ...
      {attachments && attachments.length > 0 && (
        <div className="flex gap-2 p-2 overflow-x-auto bg-[#f0f2f5] border-t border-[#d1d7db]">
          {attachments.map((file, i) => (
            <div
              key={i}
              className="relative shrink-0 w-20 h-20 bg-gray-200 rounded-md overflow-hidden border border-gray-300"
            >
      ...
      <div className="flex items-end gap-1 md:gap-2 p-2 md:p-3 bg-[#f0f2f5] border-t border-[#d1d7db] relative">
        ...
        {showEmojiPicker && (
          <div
            ref={emojiPickerRef}
            className="absolute bottom-full left-0 sm:left-2 mb-2 z-[100] shadow-2xl rounded-xl"
          >
            <EmojiPicker
        ...
        <div className="flex gap-0.5 sm:gap-1 shrink-0 pb-1.5 md:pb-2">
          <button
            className={`p-1.5 sm:p-2 rounded-full hover:bg-gray-200 transition-all duration-200 active:scale-95 emoji-btn-toggle ${showEmojiPicker ? "bg-gray-200" : ""}`}
            onClick={toggleEmojiPicker}
            type="button"
            title="Emoji"
          >
            <Smile
              className={`w-5 h-5 sm:w-6 sm:h-6 ${showEmojiPicker ? "text-teal-600" : "text-[#54656f]"}`}
            />
          </button>
          ...
          <button
            ...
            type="button"
          >
            <Plus className="w-5 h-5 sm:w-6 sm:h-6 text-[#54656f]" />
          </button>
        </div>

        <div
          onClick={handleContainerClick}
          className="flex-1 bg-white rounded-2xl md:rounded-3xl px-3 sm:px-4 py-2 shadow-sm border border-transparent focus-within:border-teal-500 transition-all min-h-[40px] sm:min-h-[45px] flex items-center cursor-text"
        >
          <textarea
            ref={textareaRef}
            value={text}
            ...
            className="w-full max-h-[100px] bg-transparent outline-none resize-none text-sm sm:text-[15px] text-[#111b21] custom-scrollbar placeholder:text-gray-500"
            rows={1}
            onKeyDown={(e) => {
        ...
        {hasContent ? (
          <button
            ref={sendButtonRef}
            onClick={handleSend}
            className="p-2.5 sm:p-3 rounded-full bg-[#00a884] hover:bg-[#008f6f] shadow-md flex items-center justify-center transition-all duration-200 active:scale-95 enter-animation shrink-0"
            type="button"
          >
          ...
          <button
            ref={sendButtonRef}
            className="p-2.5 sm:p-3 rounded-full bg-[#f0f2f5] text-[#54656f] hover:bg-gray-200 shadow-none flex items-center justify-center transition-all duration-200 cursor-default shrink-0"
            disabled
            type="button"
          >
